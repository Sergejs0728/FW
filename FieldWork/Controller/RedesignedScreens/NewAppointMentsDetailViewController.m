...
#define UnitsFeature @"Units"

#define REFRESH_LONG_TIMER 20
#define REFRESH_SHORT_TIMER 2

@interface NewAppointMentsDetailViewController ()<AppDeveiceDelegate, MFMailComposeViewControllerDelegate, iPadSamcomActionSheetDelegate, AddUnitViewControllerDelegate, MaterialUsageSelectionDelegate>
...
@property (nonatomic) CGFloat appDetailCellHeight;
...
-(void)viewWillDisappear:(BOOL)animated{
    [self.Appointment calculateTaxAmountForOwnTaxtRate:YES];
...
- (void)viewDidLoad {
    ...
    _appDetailCellHeight = 0;
    _hideCustomerInfo=[user.hide_customer_details boolValue];
...
//                       @"Drawing"];
    ...
    UIBarButtonItem *btnClose = [UIBarButtonItem barButtonWithTitle:@"Close" andBlock:^{
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:@"YES" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            ...
            [self appointmentValidationWithCompletion:^(BOOL valid) {
                if (valid) {
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateDevices) name:UPDATE_DEVIDES object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateDevices) name:UPDATE_DEVICES object:nil];
    ...
    {
        NSArray *recommendationIds = (NSArray*)weakSelf.Appointment.recommendation_ids;
        if (recommendationIds == nil) {
            recommendationIds = @[];
        }
        NSMutableArray *existing_items = [Recommendations recommendationByIds:recommendationIds];
    ...
    [self appointmentValidationWithCompletion:nil];
}
...
//                [self.gameTimer startTimer];
                ...
                [[ActivityIndicator currentIndicator] displayCompletedWithError:NSLocalizedString(@"Appointment wasn't saved", @"")];
            }
...
- (void)saveAppointment {
    ...
    if (self.Appointment.started_at_time == nil) {
        ...
        [alertController addAction:[UIAlertAction
                                    ...
                                    handler:^(UIAlertAction *action)
                                    {
                                        [weakSelf appointmentValidationWithCompletion:^(BOOL valid) {
                                            if (valid) {
        ...
        [alertController addAction:[UIAlertAction
                                    ...
                                    {
                                        [weakSelf appointmentValidationWithCompletion:^(BOOL valid) {
                                            if ([weakSelf checkReportAvailability] && valid) {
                                                [weakSelf.Appointment setStatus:wo_STATUS_COMPLETE];
                                                [weakSelf.Appointment saveAppointmentOnLocal];
                                                if ([[AppDelegate appDelegate] isReachable]) {
                                                    [[ActivityIndicator currentIndicator] displayActivity:@"Saving..."];
...
- (void)appointmentValidationWithCompletion:(void (^)(BOOL valid))completion {
    // return YES anyway to allow user save the order anyway
    if (self.Appointment.tax_rate == nil) {
        TaxRates *taxRate = [TaxRates getTaxRatesById:self.Appointment.tax_rate_id inContext:self.Appointment.managedObjectContext];
        if (taxRate) {
            self.Appointment.tax_rate = taxRate;
            if (completion) {
                completion(YES);
            }
        } else {
            if (self.Appointment.tax_rate_idValue > 0) {
                if ([[AppDelegate appDelegate] isReachable]) {
                    [TaxRates retriveTaxRate:self.Appointment.tax_rate_id withBlock:^(id result, NSString *error) {
                        if (error == nil) {
                            TaxRates *taxRate = [TaxRates getTaxRatesById:self.Appointment.tax_rate_id inContext:self.Appointment.managedObjectContext];
                            if (taxRate) {
                                self.Appointment.tax_rate = taxRate;
...
- (void)appointmentOldValidationWithCompletion:(void (^)(BOOL valid))completion __attribute__((deprecated)) {
    // some users are in infinity loop if this method using
    if (self.Appointment.tax_rate == nil) {
        // debug alert
//        [[[UIAlertView alloc] initWithTitle:@"FieldWork"
//                                    message:[NSString stringWithFormat:@"%@", [self.Appointment buildJson]]
//                                   delegate:nil
            ...
            if (self.Appointment.tax_rate_idValue > 0) {
                [UIAlertController showAlertInViewController:self
                                                   withTitle:NSLocalizedString(@"Error", @"")
                                                     message:NSLocalizedString(@"Tax rate didn't load. Please go back to calendar screen and refresh work orders", @"")
                                           cancelButtonTitle:NSLocalizedString(@"Load it now", @"")
                                      destructiveButtonTitle:NSLocalizedString(@"Continue with no tax rate", @"")
                                           otherButtonTitles:nil
                                                    tapBlock:^(UIAlertController * _Nonnull controller, UIAlertAction * _Nonnull action, NSInteger buttonIndex) {
                                                        if (buttonIndex == controller.cancelButtonIndex) {
                                                            if ([[AppDelegate appDelegate] isReachable]) {
                                                                [[ActivityIndicator currentIndicator] displayActivity:NSLocalizedString(@"Loading tax rate...", @"")];
                                                                [TaxRates retriveTaxRate:self.Appointment.tax_rate_id withBlock:^(id result, NSString *error) {
                                                                    if (error) {
                                                                        ...
                                                                        if (taxRate) {
                                                                            ...
                                                                            [[ActivityIndicator currentIndicator] displayCompletedWithError:NSLocalizedString(@"Tax rate didn't load.", @"")];
                                                                            if (completion) {
                                                                                completion(NO);
                                                                            }
                                                                        }
                ...
                [UIAlertController showAlertInViewController:self
                                                   withTitle:NSLocalizedString(@"Error", @"")
                                                     message:NSLocalizedString(@"Something went wrong. Please go back to calendar screen and refresh work orders", @"")
...
- (BOOL)checkReportAvailability {
    BOOL hasDirty = [self.Appointment hasDirtyAttachments];
    if (hasDirty) {
        __weak __typeof(self) weakSelf = self;
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Attachments uploading is in progress", @"")
                                                                       message:NSLocalizedString(@"You have to save this appointment at first", @"")
                                                                preferredStyle:UIAlertControllerStyleAlert];
        ...
        UIAlertAction *actionSave = [UIAlertAction actionWithTitle:NSLocalizedString(@"Save & Exit", @"")
                                                             style:UIAlertActionStyleDefault
                                                           handler:^(UIAlertAction * _Nonnull action) {
                                                               [weakSelf tryToSaveImmediately];
                                                           }];
...
-(void) pdfDialogForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    FWPDFForm *form = [_pdf_forms objectAtIndex:indexPath.row];
    NSArray* fields=[[form fields] array];
    BOOL openWithAcrobat=[form.use_acrobat boolValue];
    if (openWithAcrobat) {
        PDFAttachment *att = [form getRelatedAttachmentForAppointId:self.Appointment.entity_id];
        NSString *doc_path = @"";
        if (att) {
            doc_path = [att pdfFilePath];
        } else {
            doc_path = [form pdfFilePathWithAppointment:self.Appointment];
        }
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if (section == FWLineItem) {
    ...
    }else if (section == FWUnits && ([_accountFeatures containsObject:UnitsFeature])){
        return [self createHeaderViewForSection:section withTitle:@"UNITS" andButtonBlock:^{
            //addUnits
            if ([[AppDelegate appDelegate] isReachable]) {
                [self addUnit];
            } else {
        ...
        return [self createHeaderViewForSection:section withTitle:@"MATERIAL USE" andButtonBlock:^{
            ...
            ml.materialUsageSelectionDelegate = self;
        ...
        UIView *headerview = [self createHeaderViewForSection:section withTitle:@"PHOTOS" andButtonBlock:^{
            NSMutableArray *photos = [PhotoAttachment getPhotosWithAppointmentId:self.Appointment.entity_id];
            [photos filterUsingPredicate:NON_DELETED_PREDECATE()];
            if (photos.count < 12) {
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (section == FWLineItem && (self.tableView.sectionOpen != NSNotFound && self.tableView.sectionOpen == section)) {
        NSUInteger cnt = [self.Appointment.line_items filteredOrderedSetUsingPredicate:NON_DELETED_PREDECATE()].count;
        ...
        return cnt <= 0 ? 1 : cnt;
    } else if ([@[@(FWServiceInstruction),@(FWNotes),@(FWPhotos),@(FWLocationNote),@(FWUnits),@(FWDevices),@(FWSignature),@(FWEnvironment)] containsObject:[NSNumber numberWithInteger:section]] && (self.tableView.sectionOpen != NSNotFound && self.tableView.sectionOpen == section)) {
        return 1;
...
-(CGFloat)calculateAppDetailCellHeight {
...
//-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == FWAppointmentDetails) {
        if (_appDetailCellHeight == 0) {
            return [self calculateAppDetailCellHeight];
        } else {
            return  _appDetailCellHeight;
        }
...
//        }//detail
        ...
        switch (indexPath.row) {
            case 0: { //no invoice
                if (!self.Appointment.auto_generates_invoiceValue) {
                    return  44;
                }
                return  0;
            }
                break;
            case 1: { //balance forward
                if (self.Appointment.hide_invoice_informationValue || (_serviceLocaion.hide_balanceValue)) {
                    return  0;
                }
                return  44;
            }
                break;
//            case 2: { //first cell with no price
//                if (self.Appointment.auto_generates_invoiceValue) {
//                    return  0;
//                }
//                return  44;
//            }
//                break;
            case 2: { //first cell with price
                _lineItemFirstRow.hidden = !self.Appointment.auto_generates_invoiceValue;
                if (self.Appointment.auto_generates_invoiceValue) {
                    return  44;
                }
                return  0;
                ...
                break;
            default: {
                if ([[self.Appointment.line_items filteredOrderedSetUsingPredicate:NON_DELETED_PREDECATE()] count] == 0) {
                    return  0;
                }
                
                if (indexPath.row >= [[self.Appointment.line_items filteredOrderedSetUsingPredicate:NON_DELETED_PREDECATE()] count] + 3) {
                    if (self.Appointment.auto_generates_invoiceValue) {
                        return  143;
                    } else {
                        return 0;
        ...
        if (indexPath.section == FWLocationNote){
            ...
            if (indexPath.section == FWServiceInstruction){
                ...
                NSInteger height = rows * size.height + (rows - 1)*lineSpacing + 10;
                return height;
            }else if ((indexPath.section == FWDevices)||(indexPath.section == FWUnits)){
                return 100;
...
- (void)showMapsSelectorActionSheetFromSourceView:(UIView *)sourceView {
    ...
    NSString* googleMapsUrlStr = [NSString stringWithFormat: @"comgooglemaps://?daddr=%f,%f", cord.latitude, cord.longitude];
    NSURL *googleMapsURL = [NSURL URLWithString:googleMapsUrlStr];
    if ([[UIApplication sharedApplication] canOpenURL:googleMapsURL]) {
        [alertController addAction:[UIAlertAction actionWithTitle:@"Google Maps" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:googleMapsUrlStr]];
            
        }]];
    }
    NSString* wazeUrlStr = [NSString stringWithFormat: @"waze://?ll=%f,%f&navigate=yes", cord.latitude, cord.longitude];
    NSURL *wazeURL = [NSURL URLWithString:wazeUrlStr];
    if ([[UIApplication sharedApplication] canOpenURL:wazeURL]) {
        [alertController addAction:[UIAlertAction actionWithTitle:@"Waze Maps" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:wazeUrlStr]];
        }]];
    }
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    ...
    if ((section==FWUnits)&&(![_accountFeatures containsObject:UnitsFeature])) {
        return 0;
    }
    if([@[@(FWLineItem),@(FWServiceInstruction),@(FWLocationNote),@(FWEnvironment),@(FWUnits),@(FWPDFForms),@(FWNotes),@(FWScheme),@(FWMaterialUse),@(FWPhotos),@(FWDevices),@(FWSignature), @(FWRecommendation), @(FWConditions)] containsObject:[NSNumber numberWithInteger:section]]){
        return 38;
    }
    else {
        return 0;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == FWAppointmentDetails) {
        ...
        __weak AppDetailCell *weakCell = cell;
        cell.drivingDirectionAction = ^(NSInteger tag){
            [self showMapsSelectorActionSheetFromSourceView:weakCell.buttonDrivingDirections];
        };
        ...
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
    }else if (indexPath.section == FWUnits){
        AppUnitsCell  *cell = (AppUnitsCell *)[self createcellWithtableView:tableView andIndexPath:indexPath];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
...
-(id)createcellWithtableView:(UITableView *)tableView andIndexPath:(NSIndexPath *)indexpath{
    ...
    if (indexpath.section == FWAppointmentDetails) {
        ...
        cell.emailAction = ^(NSString *email){
            if ([email length]) {
                [self displayMailComposerSheetWithString:email filePath:NO];
            }
            
        };
        __weak NewAppointMentsDetailViewController *weakSelf = self;
        cell.phoneAction = ^(NSString *phone) {
            if ([phone length]) {
                weakSelf.customerPhone = phone;
                UIAlertView *alrt=[[UIAlertView alloc]initWithTitle:@"Select Action" message:nil delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Call",@"Text", nil];
                [alrt show];
            }
        };
            ...
            switch (indexpath.row) {
                ...
                default: {
                    if (indexpath.row < [[self.Appointment.line_items filteredOrderedSetUsingPredicate:NON_DELETED_PREDECATE()] count] + 3) {
                        ...
                        LineItem *line_item = [[[self.Appointment.line_items filteredOrderedSetUsingPredicate:NON_DELETED_PREDECATE()] array] objectAtIndex:(indexpath.row - 3)];
...
//                            }
                            ...
                            [weakSelf.Appointment setDiscount_amount:[NSNumber numberWithFloat:discount]];
                            [weakSelf.Appointment calculateTaxAmountForOwnTaxtRate:YES];
                            
                            return weakSelf.Appointment;
                        };
                        if (self.Appointment.hide_invoice_informationValue) {
    ...
    else if (indexpath.section == FWServiceInstruction) // LocationNote
    {
        ...
        if (indexpath.section == FWPDFForms) // FWPDFForm
    {
        ...
        static NSString *identifier = @"UnitsIdentifier";
        AppUnitsCell *cell = (AppUnitsCell*) [tableView dequeueReusableCellWithIdentifier:identifier];
        ...
        cell.viewAllUnits= ^{
            UnitListViewController* controller= [UnitListViewController initWithAppointment:self.Appointment];
            [self.navigationController pushViewController:controller animated:YES];
        };
        if (cell == nil)
        {
            NSArray *topLevelObject = [[NSBundle mainBundle] loadNibNamed:@"AppUnitsCell" owner:self options:nil];
            for(id appcell in topLevelObject)
            {
                if([appcell isKindOfClass:[UITableViewCell class]])
                {
                    cell = (AppUnitsCell*) appcell;
                    cell.addUnit = ^{
                        if ([[AppDelegate appDelegate] isReachable]) {
                            [self addUnit];
                        } else {
                    ...
                    cell.viewAllUnits= ^{
                        UnitListViewController* controller= [UnitListViewController initWithAppointment:self.Appointment];
                        controller.unitsList=[NSMutableArray arrayWithArray:[_serviceLocaion.flats allObjects]];
                        [self.navigationController pushViewController:controller animated:YES];
                    };
                }
            }
        }
        NSArray* units=[_serviceLocaion.flats allObjects];
        NSString* totalString=[NSString stringWithFormat:@"%lu",(unsigned long)units.count];
        [cell.totalCountLabel setText:totalString];
        
        NSNumber *servicedUnitStatusId = [UnitStatus servicedUnitStatusId];
        [cell.servicedCountLabel setText:@"0"];
        if (servicedUnitStatusId) {
            NSArray *serviced = [UnitInspection unitInspectionsWithAppointmentId:self.Appointment.entity_id unitStatusId:servicedUnitStatusId deleted:NO];
            [cell.servicedCountLabel setText:[NSString stringWithFormat:@"%lu", serviced.count]];
        }
...
-(void)addFloor{
    ...
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"OK"
                                                     ...
                                                     handler:^(UIAlertAction *action) {
                                                         ...
                                                         if (floor.length==0) {
                                                             NSString *message = NSLocalizedString(@"Set name", @"");
                                                             ...
                                                             StageModel *scheme = [StageModel stageByFloor:floor serviceLocation:[self.Appointment getServiceLocation]];
                                                             if (scheme) {
                                                                 NSString *message = NSLocalizedString(@"Already exists", @"");
                                                                 UIAlertController *innerAlert = [UIAlertController alertControllerWithTitle:@"Error"
                                                                                                                                     message:message
                                                                                                                              preferredStyle:UIAlertControllerStyleAlert];
                                                                 UIAlertAction *innerCancelAction = [UIAlertAction actionWithTitle:@"OK"
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == FWLineItem){
        ...
        if (indexPath.row < 3 || indexPath.row >= [[self.Appointment.line_items filteredOrderedSetUsingPredicate:NON_DELETED_PREDECATE()] count] + 3) {
            return;
        }
        LineItem *info = [[[self.Appointment.line_items filteredOrderedSetUsingPredicate:NON_DELETED_PREDECATE()] array] objectAtIndex:(indexPath.row - 3)];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        if (indexPath.section == FWLineItem) {
            // Line Item
            NSArray *line_items = [[self.Appointment.line_items filteredOrderedSetUsingPredicate:NON_DELETED_PREDECATE()] array];
            ...
            LineItem *info = [line_items objectAtIndex:indexPath.row - 3];
            ...
            if (_stages.count>0) {
                ...
                UIAlertAction *okDeleteAction = [UIAlertAction actionWithTitle:@"OK"
                                                                       ...
                                                                       handler:^(UIAlertAction *action) {
                                                                           ...
                                                                           if (stage.entity_idValue > 0) {
                                                                               stage.deleted = @(YES);
                                                                               [stage.managedObjectContext MR_saveOnlySelfAndWait];
                                                                           } else {
                                                                               [stage.managedObjectContext MR_saveOnlySelfAndWait];
                                                                               [[NSManagedObjectContext MR_defaultContext] deleteObject:stage];
                                                                           }
                                                                           [self loadStages];
                                                                       }];
...
- (UITableViewCellEditingStyle)tableView:(UITableView *)tableView editingStyleForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == FWLineItem) {
        NSSet *lineItems = [self.Appointment.line_itemsSet filteredOrderedSetUsingPredicate:NON_DELETED_PREDECATE()];
        if (lineItems.count > 0 && (indexPath.row >= 3 || indexPath.row < lineItems.count + 3)) {
    ...
    [alertController addAction:[UIAlertAction
                                ...
                                handler:^(UIAlertAction *action)
                                {
                                    ...
                                    if ([photo.entity_status isEqualToNumber:c_ADDED]) {
                                        [self.Appointment.photo_attachmentsSet removeObject:photo];
...
- (void) addUnitViewControllerDidFinish:(Unit*)unit {
    if (unit) {
        [_serviceLocaion addFlatsObject:unit];
        [self refreshObjects];
...
- (BOOL) checkIfMovesToGroup {
    ...
    for (DeviceArea *area in deviceAreas) {
        for (CustomerDevice *device in traps) {
            if (device.device_area_id == area.entity_id) {
                flag = TRUE;
                return flag;
            }
        }
...
-(void)viewAllDevices{
    ...
    if ([self checkIfMovesToGroup]) {
        TrapGroupListController *trapGroupList = [TrapGroupListController initWithAppointment:self.Appointment];
        [self.navigationController pushViewController:trapGroupList animated:YES];
    } else {
...
- (void)barcodeScanerViewControllerDidScan:(NSString *)barcode {
    [_reader dismissViewControllerAnimated:YES completion:^{
        CustomerDevice *device = [CustomerDevice deviceByBarcode:barcode andService_location:self.Appointment.service_location_id];
        if (device == nil) {
            ...
            NSSet *temp = [self.Appointment.inspection_records filteredSetUsingPredicate:[NSPredicate predicateWithFormat:@"barcode == %@", barcode]];
            ...
            TrapInspectionViewController *vc = [TrapInspectionViewController initWithAppointment:self.Appointment withTrap:device];
...
//    NSString *filePath = [mainBundle pathForResource:@"points" ofType:@"csv"];
                ...
                if (doc_path.length > 0) {
                    ...
                    if (myData) {
                        ...
                        if (att) {
                            ...
                            [form downloadWithAppointment:self.Appointment andCompletionBlock:^(FWRequest *request) {
                                counter--;
                                NSData *myData = [NSData dataWithContentsOfFile:doc_path];
                                if (myData) {
                                    [picker addAttachmentData:myData
...
- (void)materialUsageSelectedWithMaterialUsage:(MaterialUsage *)materialUsage{
    //    if (![self.Appointment.material_usagesSet containsObject:materialUsage]) {
    //        [self.Appointment addMaterial_usagesObject:materialUsage];
    //    }
    //    [[NSManagedObjectContext MR_defaultContext] MR_saveToPersistentStoreAndWait];
    // ensure that the object in data base
    [materialUsage.managedObjectContext MR_saveOnlySelfAndWait];
    // update the relation
    if (![self.Appointment.material_usagesSet containsObject:materialUsage]) {
        [MagicalRecord saveWithBlockAndWait:^(NSManagedObjectContext * _Nonnull localContext) {
            Appointment *app = [self.Appointment MR_inContext:localContext];
            MaterialUsage *mu = [materialUsage MR_inContext:localContext];
            [app addMaterial_usagesObject:mu];
        }];
    }
    //refresh
    [self refreshObjects];
...
-(void)addUnit{
    ...
    controller.serviceLocation = _serviceLocaion;
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    NSString* phoneNumber=_customerPhone;
    ...
    if (buttonIndex == 1) {
        ...
        NSString *smsURL = [@"sms://" stringByAppendingString:phoneNumber];
