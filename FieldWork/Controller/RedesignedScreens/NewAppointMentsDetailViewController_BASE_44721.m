...
- (void)viewDidLoad {
    ...
    _pdf_forms=[self.Appointment.pdf_forms allObjects];
     ...
     //[AppDelegate appDelegate].containerVC.panMode =  MFSideMenuPanModeNone;
...
-(void)loadStages{
    ...
    ServiceLocation* serviceLocation=[ServiceLocation MR_findFirstWithPredicate:[NSPredicate predicateWithFormat:@"ANY entity_id == %@", serviceLocationId]];
    if((serviceLocation.floors)||(serviceLocation.floors.count>0)||(serviceLocation.floors.count)){
        ...
        stages=[stages filteredArrayUsingPredicate:predicate];
        if (!self.buildings) {
        ...
        NSOrderedSet* tempSet=[NSOrderedSet orderedSetWithArray:tempBuildingArray];
        self.buildings =[NSMutableArray arrayWithArray: [tempSet array]];
        [self refreshObjects];
    ...
    if (fields) {
        if (fields.count>0) {
            UIAlertController *alertController = [UIAlertController
                                                  alertControllerWithTitle:@"PDFForm"
                                                  message:@"Do you want to forms in FieldWork?"
                                                  preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction *cancelAction = [UIAlertAction
                                           actionWithTitle:NSLocalizedString(@"No", @"Open in external app")
                                           style:UIAlertActionStyleCancel
                                           handler:^(UIAlertAction *action)
                                           {
            ...
            UIAlertAction *yesAction = [UIAlertAction
                                        actionWithTitle:NSLocalizedString(@"Yes", @"Open FieldWork forms")
                                        style:UIAlertActionStyleDefault
                                        handler:^(UIAlertAction *action)
                                        {
...
//                                            pdfController.fieldsBlock=^(PDFForm* form, PDFAttachment* attachment){
//                                                NSMutableArray* fields=[NSMutableArray new];
//                                                NSArray* fieldsArray=[[attachment field_values] array];
//                                                for(PDFField* field in fieldsArray ){
//                                                    if (field.field_value) {
//                                                        NSMutableDictionary* item= [NSMutableDictionary dictionaryWithDictionary:@{@"pdf_name":field.pdf_name,@"value":field.field_value,@"type":field.field_type,@"default_value":field.default_value,@"required":field.required,@"label":field.label,@"name":field.name}];
//                                                        if (field.options) {
...
//                                                    }
//                                                }
//                                                NSDictionary* innerJson=@{@"pdf_form_id":form.entity_id,
//                                                                          @"field_values":fields};
//                                                NSDictionary* json=@{@"attachment":innerJson};
//                                                NSString* workOrderId=[NSString stringWithFormat:@"%@",self.Appointment.entity_id];
...
//                                                Appointment * appointment= [Appointment getById:self.Appointment.entity_id];
//                                                _pdf_forms=[appointment.pdf_forms allObjects];
//                                                _pdf_attachments=[appointment.attachments allObjects];
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    __weak __typeof(self) weakSelf = self;
    if (section == FWLineItem) {
        if ([self.Appointment.hide_invoice_information boolValue] == YES) {
        ...
        UIView *headerview = [self createHeaderViewForSection:section withTitle:@"PHOTOS" andButtonBlock:^{
         ...
         return headerview;
    }else if (section == FWDevices){
        return [self createHeaderViewForSection:section withTitle:@"DEVICES" andButtonBlock:nil];
    }else if (section == FWScheme){
        return [self createHeaderViewForSection:section withTitle:@"SITE DRAWING" andButtonBlock:^{
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Add building?"
                                                                           message:@"Set building title"
                                                                    preferredStyle:UIAlertControllerStyleAlert];
            ...
            UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"OK"
                                                               style:UIAlertActionStyleDefault
                                                             handler:^(UIAlertAction *action) {
                                                                NSString* building=[alert.textFields objectAtIndex:0].text;
                                                                [self.buildings addObject:building];
//                                                                 if (self.buildings.count>0) {
                                                                FloorPlansViewController *floorPlans = [[FloorPlansViewController alloc] initWithNibName:@"FloorPlansViewController" bundle:nil];
                                                                [floorPlans setBuilding:building];
                                                                [floorPlans setServiceLocationId:self.Appointment.service_location_id];
                                                                 floorPlans.block=^(){
                                                                     [self loadStages];
                                                                 };
                                                                [self.navigationController pushViewController:floorPlans animated:YES];
//                                                                 }
                                                             }];
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (section == FWLineItem && (self.tableView.sectionOpen != NSNotFound && self.tableView.sectionOpen == section)) {
        ...
        NSInteger cnt=self.buildings.count;
        return cnt > 0 ? cnt : 1;
    }else if (section == FWEnvironment ) {
        return 0;
...
//        if (self.Appointment.photo_attachments.count == 0 || self.Appointment.photo_attachmentsSet.count == 0)
//        {
//            return [self getNoItemCell:@"No photos added."];
//        }
...
//        [cell setPhotos:[[[self.Appointment.photo_attachments filteredSetUsingPredicate:NON_DELETED_PREDECATE()] allObjects] mutableCopy] withDeleteBlock:^(PhotoAttachment *photo) {
...
//            if (![[User getUser].hide_customer_details boolValue]) {
//                CustomerDetailViewController *controller = [CustomerDetailViewController initWithAppointment:self.cust];
//                controller.isHidden = YES;
...
//        static NSString *identifier = @"AppWeatherCelldentifier";
        ...
        [cell initializeWithTemperature:self.Appointment.temperature windDirection:self.Appointment.wind_direction  windSpeed:self.Appointment.wind_speed];
        return cell;
        ...
        if ([self.Appointment.hide_invoice_information boolValue]) {
            ...
            } else if (indexpath.row == [[self.Appointment.line_items filteredSetUsingPredicate:NON_DELETED_PREDECATE()] count] + 1){
                __weak typeof(self) weakSelf = self;
...
//                    [weakSelf calculateLineItems];
    ...
    else if (indexpath.section == FWServiceInstruction) // ServiceInstruction
    {
    ...
    else if (indexpath.section == FWLocationNote) // LocationNote
    {
        static NSString *identifier = @"ServiceIdentifier";
        AppServiceCell *cell = (AppServiceCell*) [tableView dequeueReusableCellWithIdentifier:identifier];
        if (cell == nil)
    ...
    }else if (indexpath.section == FWScheme) // Scheme
    {
        ...
        if (self.buildings) {
//            NSArray* buildings=[[self.Appointment.getServiceLocation buildings] array];
            if (self.buildings) {
                NSString* building=[self.buildings objectAtIndex:indexpath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == FWLineItem){
        ...
        if (self.buildings) {
            if (self.buildings.count>0) {
                NSString* building= [self.buildings objectAtIndex:row];
                FloorPlansViewController *floorPlans = [[FloorPlansViewController alloc] initWithNibName:@"FloorPlansViewController" bundle:nil];
                floorPlans.block=^(){
                    NSIndexSet* indexSet=[NSIndexSet indexSetWithIndex:FWScheme];
                    [self.tableView reloadSections:indexSet withRowAnimation:UITableViewRowAnimationNone];
...
- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([@[@(FWLineItem),@(FWMaterialUse)] containsObject:[NSNumber numberWithInteger:indexPath.section]]) {
