...
@private
	...
	__strong UIImage *currentSignatureImage_;
	float lineWidth_;
...
{
    if ([delegate respondsToSelector:@selector(showHideLable:)]) {
        [delegate showHideLable:touch];
    }
}
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	//NSLog(@"touchesMoved : %f - %f - %f - %f", touchLocation.x, touchLocation.y, lastTapPoint_.x, lastTapPoint_.y);
}
...
-(UIImage *)getSignatureImage {
    ...
    //float minX = 9999999.0f, minY = 99999999.0f, maxX = 0.0f, maxY = 0.0f;
